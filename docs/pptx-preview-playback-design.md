...
- 不属于：`0.2.0` 候选版本范围
- 主方案：以 `@aiden0z/pptx-renderer` 作为静态解析和渲染底座，在项目内实现播放层

本文记录产品目标、选型结论、包边界、播放模型、实施顺序和验收门槛。本文不表示 PPTX 已经可用，也不把第三方项目的功能说明当作本项目的验收证据。
...
解包、XML 读取和时间安排编译应进入 Worker。第三方渲染器依赖浏览器 DOM 的部分留在主线程。Worker 只返回静态模型所需的可传输数据和播放模型；若第三方模型暂时不能可靠传输，阶段零必须测量主线程解析成本，并决定是否维护 Worker 适配，不允许静默放弃 Worker 目标。
...
性能预算不得在没有正式基线时随意填写。阶段零完成后，把批准环境、素材、冷启动、首次可见页、切页、内存峰值和释放后内存写入 `docs/testing/performance-baseline.md`。
...
1. 正式素材的支持内容没有整页空白或大面积错位；
2. 隐藏页在浏览模式可识别，在演示模式默认跳过；
3. 每个页面对象都能用稳定形状 ID 定位；
4. 支持的淡入、出现、消失和简单移动能够按原单击顺序执行；
5. “与上一动画同时”和“上一动画之后”顺序正确；
6. 页面切换方向和持续时间正确；
7. 同一页面连续重置十次结果一致；
8. 快速切换文件不会出现旧文件回写；
9. 解析失败、超限和未支持项有明确错误或警告；
10. 独立消费项目可以从真实压缩包使用公开入口。

播放时间误差的批准阈值先以不超过 100 毫秒为试验目标，最终数值必须由正式浏览器基线确认。静态视觉阈值同样先建立基线，再写入发布门禁，不能引用第三方项目的测试数量代替本项目验收。

## 16. 实施顺序

### 阶段 0：独立试验

- 在正式包之外建立 `experiments/pptx-playback/` 或独立同级目录；
- 固定第三方版本和提交；
- 使用正式 PPTX 素材验证静态画面；
- 为渲染对象补充稳定 DOM 标记；
- 从同一份 `PptxFiles` 读取隐藏页、页面切换和基础 `p:timing`；
- 只实现淡入、出现、消失、简单移动和一种页面切换；
- 记录素材、截图、录屏、问题和停止结论。

阶段零不修改 DOCX、XLSX 的公开包，也不把试验代码伪装成正式 PPTX 能力。

### 阶段 1：建立包边界

- 新建 `pptx-core` 和 `vue-pptx`；
- 接入 Runtime、资源限制、取消、Worker 和 session；
- 固定错误、警告和公开类型；
- 建立真实压缩包消费用例。

### 阶段 2：静态浏览

- 页面列表、单页、缩略图、搜索、缩放和备注；
- 隐藏页标记；
- 大文件窗口化与资源释放；
- 正式静态视觉基线。
...
阶段五中的每项都需要独立设计和素材门槛，不因基础播放器完成而自动进入发布承诺。
...
1. `@aiden0z/pptx-renderer` 的固定版本和补丁维护方式；
2. 稳定 DOM 标记是否能被上游接受；
3. 第一期批准的动画和页面切换清单；
4. Morph 是否进入产品路线，还是长期列为未支持；
5. 外部音视频是否默认完全禁用；
6. 浏览器和设备支持范围；
7. 正式性能预算和视觉差异阈值；
8. PPTX 是否作为两个新公开包发布，还是先保留为试验包。
