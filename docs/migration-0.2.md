...
如需自定义 PDFium 地址，从 `@arcships/vue-pdf/assets/pdfium.wasm` 取得包内资源，或通过公开 `pdfiumWasmUrl`/Runtime 配置覆盖。严格内容安全策略需要允许应用实际采用的 Worker 和同源 WASM 资源。
...
| `@arcships/vue-extend` | 拆分为 `@arcships/vue-pdf` 和 `@arcships/vue-ui` | `0.2.0` 起不再发布 `vue-extend`；PDF 组件迁移到 `vue-pdf`，通用组件迁移到 `vue-ui` |
