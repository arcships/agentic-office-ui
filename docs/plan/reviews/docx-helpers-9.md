...
- **Impact:** The selection/section-mutation helpers that downstream `docx-composables` needs (alignment #30 `historyRestoreRequest` nonce-driven restore, #31 `selectionSession`, section header/footer editing) are implemented but unreachable. A consumer importing `@arcships/docx-core` cannot use them. Must add the three `export *` lines to the barrel and update the Completed/Pending split.
...
| Typecheck (docx-core) | ✅ Pass | `pnpm --filter @arcships/docx-core typecheck` exits 0. All `.ts` under `src/` compiled (incl. orphans, since `include: ["src"]`). |
