# Review: xlsx-composables-split-9

Date: 2026-07-08
Task: xlsx-composables-split（架构设计 `docs/xlsx-migration-architecture.md` §2.2 / §三 步骤4 / §4.4）
Branch: task/docx-remigration
HEAD: 6b52a1d
前置 review: `xlsx-composables-split-1.md`～`-8.md`（全部 blocked）

## 结论

**blocked** —— HEAD `6b52a1d`（"fix #8"）相比 review-8（`0992f59`）**无任何代码改动**：该 commit 唯一变更是新增上一轮 review 的 markdown 文件（`docs/plan/reviews/xlsx-composables-split-8.md`，+250 行）。`git log --oneline 0992f59..HEAD` 仅此一个 commit，`git diff 0992f59..HEAD -- packages/vue-xlsx/src/` 为空。

review-8 记录的全部 blocking 问题一字未改：
...
这是连续第九次 review 同一核心问题。review-8 给出的 5 条建议（剪切迁移、补 controller 文件、补 barrel + 接线、清理 monolith debug 日志、验证）全部未落实，连第一步"剪切迁移"都未启动。review-8→review-9 之间零代码进展。
...
```
$ git diff --stat 0992f59..HEAD -- packages/vue-xlsx/src/composables.ts
（空）
...
```
$ git rev-parse HEAD
6b52a1d4cd0d38ce6b4d8636cf40556bd8d4f9f2

$ git log --oneline 0992f59..HEAD          # review-8 → review-9 之间仅 1 个 commit
6b52a1d xlsx-composables-split: fix #8
$ git show --stat 6b52a1d                    # 该 commit 仅新增 review-8 markdown，无代码
 docs/plan/reviews/xlsx-composables-split-8.md | 250 ++++++++++++++++++++++++++
 1 file changed, 250 insertions(+)

$ git diff --stat 0992f59..HEAD -- packages/vue-xlsx/src/   # 代码零改动
（空）
...
连续九次 review 同一核心问题（monolith 未拆、子文件是复制副本/平行死代码）未解决。review-8→review-9 之间**零代码进展**——唯一的 commit 仅添加了 review-8 的 markdown 文件，`composables.ts` 与 `composables/*.ts` 均无任何改动。

## 建议下一步

review-8 的 5 条建议一字未落实，仍然适用：

1. **剪切迁移**（review-4/5/6/7/8 建议 1，未落实）：按 §2.2 的 8 模块把 `composables.ts` 代码**移出**，monolith 删除本地实现并改为从子模块 `import`，最终 `composables.ts` 消失或仅剩极薄入口。当前是"复制 + 加平行架构"，必须改为"剪切"。
2. **补 controller 文件**（review-4/5/6/7/8 建议 2，未落实）：`composables/useXlsxViewerController.ts`，把 `useXlsxViewerController`（monolith:1899）及其 setup/return（monolith:4606-4723）移入，return 改为聚合各 `createXxxDomain(ctx)` 的产物。
3. **补 barrel + 接线**（review-4/5/6/7/8 建议 3，未落实）：`composables/index.ts`，`src/index.ts` 从 `./composables`（解析到 barrel）导入。接入后 F3 死代码问题自然消除。
...
关键判断：本任务已连续九轮 blocking 且 review-8→review-9 零代码进展。继续重复 review 不会推进拆分；建议将任务退回执行方，明确要求"剪切迁移而非复制"这一唯一正确路径后再提交，避免第十次 review 同一问题。
