...
- `62dba91 docx-helpers: fix #6` (batch 4 — added `page-measurement.ts`, 689 lines; reviewed in #7)
- `4326037 docx-helpers: fix #7` (only adds `docx-helpers-7.md`; **no helpers source code changed**)

HEAD is `4326037`; working tree clean. Since review #7, **no new helpers source landed** — commit `4326037` adds only this review's predecessor document. The deliverable surface remains 27 content modules + barrel, ~12931 lines (excl. barrel), against a plan of 40 content files / ~21200 lines. Task is ~68% complete by file count (27 of 40).
...
### F1 — P1 / blocking — 14 of 40 planned helper modules still missing (carried from #3–#7, unchanged this cycle)
...
- **Code:** [packages/docx-core/src/editor/helpers/](../../packages/docx-core/src/editor/helpers/) — 27 modules delivered (excl. barrel), ~12931 lines
...
- **Impact:** The pagination-plan trio, pretext-build/measure, table-height, tracked-changes, and selection helpers are the critical path for `docx-composables` (which depends-on `docx-helpers`). The layout layer already declares `estimateTableRowHeightsPx` as a callback parameter and invokes it, but the helpers layer that must supply the real implementation has not delivered it — the callback contract is unbacked by a helper. Alignment items #22 (measurement-driven iteration), #23 (pretext variable-width integration), and #32 (tracked-changes from-model derivation) are explicitly "必须复刻" and remain without implementations. `vue-docx` is still a stub (`packages/vue-docx/src/` has only `composables.ts` + `index.ts` + `env.d.ts`; no `composables/` directory), confirming `docx-composables` cannot start. The barrel [index.ts:1](../../packages/docx-core/src/editor/helpers/index.ts#L1) still says "modular split in progress" and lists the missing modules as Pending — honest, but confirms the task is not complete.
  ...
  Note on the batch model: the workflow doc allows docx-helpers to land in batches, and this is review #8 of an explicitly incremental task. The blocking verdict is scoped to the *task* (`docx-helpers`) not being completable until the remaining 14 modules land. Eight successive reviews (#1–#8) have now blocked on this same F1; the deliverable surface grew 20 → 25 → 26 → 26 → 26 → 26 → 27 across batches #1–#7, then **0** this cycle (#8). The critical-path modules (pagination-plan, pretext-build/measure, table-height, tracked-changes, selection) remain.

### F2 — P3 / non-blocking — `paragraph-geometry.ts` exceeds the 1000-line hard constraint (carried from #1–#7)
...
### F3 — P3 / non-blocking — barrel + README still mark task "in progress" / pending (carried from #3–#7)
...
- **Code:** [index.ts:1,20-28](../../packages/docx-core/src/editor/helpers/index.ts#L1) ("modular split in progress", modules listed Pending — the Pending comment block at lines 20-28 is stale: it lists 17 modules that are already delivered and exported (`header-footer`, `page-measurement`, `paragraph-inspect`, `paragraph-geometry`, `paragraph-tracked`, `drop-cap`, `letterhead`, `line-height`, `style-to-css`, `xml-parsing`, `synthetic-textbox`, `field-helpers`, `numbering`, `table-utils`, `table-utils-extra`, `text-mutation`, `state`) alongside the 14 that are genuinely missing); [plan/README.md:44](../README.md#L44) (`docx-helpers … pending`)
- **Detail:** Honest documentation of partial state, consistent with the batch model, but the Pending comment list is now misleading — it does not distinguish delivered-from-missing. Once the remaining 14 modules land, the barrel header, the Completed/Pending split, and the README status should all flip to `done`; in the meantime the Pending list should be pruned to the actually-missing 14.
- **Impact:** Scope tracking only.

### F4 — P3 / non-blocking — `style-to-css.ts` duplicates the `ParagraphTrackedInlineChange` forward type (carried from #1–#7)
...
### F5 — P3 / non-blocking — helpers import from the viewer layer, inverting the documented dependency direction (carried from #1–#7)
...
3. **F3** (cleanup alongside): Prune the barrel Pending comment block to the actually-missing 14 modules now; flip barrel header + Completed/Pending list and README status to `done` once F1 lands.
...
F1 persists: 14 of 40 planned modules remain absent, including the pagination-plan trio, pretext-build/measure, table-height, tracked-changes, and selection helpers that carry "必须复刻" alignment obligations (#22, #23, #32) and are prerequisites for the downstream `docx-composables` task (still a stub). Since review #7, **zero** new helper source landed — the only intervening commit (`4326037`) adds review #7's document and touches no `packages/docx-core/` files. The delivered code remains high quality — typecheck green, build succeeds, graph acyclic, imports relative, zero stubs/mocks/TODOs, no React leakage. Re-verify after the remaining 14 modules are delivered.
