...
| Typecheck (docx-core) | ✅ Pass | `pnpm --filter @arcships/docx-core typecheck` exits 0. All `.ts` under `src/` compiled. |
| Build (docx-core) | ✅ Pass | `pnpm --filter @arcships/docx-core build` (tsup) succeeds — ESM (483.77 KB) + DTS (161.83 KB). |
