...
- 跨包引用使用 `@extend-ai/docx-core`（workspace 协议）
- 内部文件间引用使用相对路径
...
| 包 | 命令 | 结果 |
|---|---|---|
| `@extend-ai/docx-core` | `pnpm run --filter @extend-ai/docx-core typecheck` | ✅ 通过 |
| `@extend-ai/vue-docx` | `pnpm run --filter @extend-ai/vue-docx typecheck` | ✅ 通过 |
