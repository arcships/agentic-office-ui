# Review: xlsx-composables-split-8

Date: 2026-07-08
Task: xlsx-composables-split（架构设计 `docs/xlsx-migration-architecture.md` §2.2 / §三 步骤4 / §4.4）
Branch: task/docx-remigration
HEAD: 0992f59
前置 review: `xlsx-composables-split-1.md`～`-7.md`（全部 blocked）

## 结论

**blocked** —— HEAD `0992f59`（"fix #7"）相比 review-7（`667787a`）**无任何代码改动**：该 commit 唯一变更是新增本任务 review-7 的 markdown 文件（`docs/plan/reviews/xlsx-composables-split-7.md`，+232 行）。`git log --oneline 667787a..HEAD` 仅此一个 commit，`git diff 667787a..HEAD -- packages/vue-xlsx/src/` 为空。

review-7 记录的全部 blocking 问题一字未改：
...
这是连续第八次 review 同一核心问题。review-7 给出的 5 条建议（剪切迁移、补 controller 文件、补 barrel + 接线、清理 monolith debug 日志、验证）全部未落实，连第一步"剪切迁移"都未启动。review-7→review-8 之间零代码进展。
...
```
$ git diff --stat 667787a..HEAD -- packages/vue-xlsx/src/composables.ts
（空）
...
3. `src/index.ts:2,4,5` 仍 `from "./composables"`，解析到 monolith `composables.ts`，子模块无人引用：
   ```
   $ grep -n 'from' packages/vue-xlsx/src/index.ts
   1:import { defineComponent, h } from "vue"
   2:import { useXlsxViewerController } from "./composables"
   ```
4. monolith 的 `useXlsxViewerController` 从不构造 `XlsxControllerContext`、从不调用任何 domain factory，其 return（monolith:4606-4723）直接引用内联局部函数：
   ```
...
- 要点 23 双层历史：`pushHistoryEntry`/`applyCellMutationState`/`isApplyingHistoryRef`（monolith:1937,3272,3285），snapshot/cell-edit/range-edit 三类齐全。
...
### F7 — monolith 侧仍有 3 处 debug 日志未清理（既有问题，接入时需一并处理）
...
3692:    console.info("[react-xlsx debug] updateWorkbookChartAnchor", { ... });
```
...
```
$ git rev-parse HEAD
0992f59fc26acfc18b2c0803ea722d43b2df2a37

$ git log --oneline 667787a..HEAD          # review-7 → review-8 之间仅 1 个 commit
0992f59 xlsx-composables-split: fix #7
$ git show --stat 0992f59                    # 该 commit 仅新增 review-7 markdown，无代码
 docs/plan/reviews/xlsx-composables-split-7.md | 232 ++++++++++++++++++++++++++
 1 file changed, 232 insertions(+)

$ git diff --stat 667787a..HEAD -- packages/vue-xlsx/src/   # 代码零改动
（空）
...
连续八次 review 同一核心问题（monolith 未拆、子文件是复制副本/平行死代码）未解决。review-7→review-8 之间**零代码进展**——唯一的 commit 仅添加了 review-7 的 markdown 文件，`composables.ts` 与 `composables/*.ts` 均无任何改动。

## 建议下一步

review-7 的 5 条建议一字未落实，仍然适用：

1. **剪切迁移**（review-4/5/6/7 建议 1，未落实）：按 §2.2 的 8 模块把 `composables.ts` 代码**移出**，monolith 删除本地实现并改为从子模块 `import`，最终 `composables.ts` 消失或仅剩极薄入口。当前是"复制 + 加平行架构"，必须改为"剪切"。
2. **补 controller 文件**（review-4/5/6/7 建议 2，未落实）：`composables/useXlsxViewerController.ts`，把 `useXlsxViewerController`（monolith:1899）及其 setup/return（monolith:4606-4723）移入，return 改为聚合各 `createXxxDomain(ctx)` 的产物。
3. **补 barrel + 接线**（review-4/5/6/7 建议 3，未落实）：`composables/index.ts`，`src/index.ts` 从 `./composables`（解析到 barrel）导入。接入后 F3 死代码问题自然消除。
...
关键判断：本任务已连续八轮 blocking 且 review-7→review-8 零代码进展。继续重复 review 不会推进拆分；建议将任务退回执行方，明确要求"剪切迁移而非复制"这一唯一正确路径后再提交，避免第九次 review 同一问题。
