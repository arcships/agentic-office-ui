...
> 状态：Chromium `149.0.7827.55` 性能预算已批准，作为独立性能检查保留，不再阻断 npm 发布。

本页对应 `P3-PERF-BASELINE-01` 和 `BB-PERF-XLSX`。首次连续三轮只建立待批准基线，不得把结果写成候选发布性能通过。

性能检查继续通过 `pnpm test:performance` 单独运行并保存三轮数据。GitHub 共享机器的时间指标在相同系统和浏览器下仍有明显抖动，因此 `pnpm test:release` 不再执行该套件，发布候选也不再要求附带性能摘要。类型、构建、功能黑盒、压力、包内容、外部安装、三浏览器兼容和重复打包仍然是发布阻断条件。
...
- `approved`：后续运行按每项已批准中位数加 `10%` 检查。CSS 原始和 gzip 体积在 `500,000` 字节以内不因相对增长阻断；超过该绝对值后继续执行相对基线检查。非零的毫秒指标至少允许 `10ms` 的绝对调度波动；零长任务等零基线仍必须保持为零。超过预算即失败并要求人工分析。
...
当前批准配置：`tests/blackbox/performance-baseline.json`，SHA-256 为 `a309ac28a1e00e571803d9d82fa64fc4ee7e4d91179851d44c151d77bf764454`。T6 在纳入跨格式引用包、四种 Surface 适配和选择控件后重新采集并批准资产预算；正式三轮复验位于 `output/acceptance/de7b2ba31fd538645627abd176ddb7d145d2f80e/P3-PERF-BASELINE-01/20260714T045609Z/performance/`，总摘要结果 `PASS`，SHA-256 为 `8ad78cce209a6ca0742bd85835af053d40a83b9f192d5521e3660cc05f488b91`。本轮滚动 P95 中位数约 `9.6ms`，滚动长任务、生命周期节点/监听器增长、回 Home 后活动 Worker 和 Object URL 均为 `0`。
