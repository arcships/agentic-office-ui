...
cd "$CONSUMER"
npm pkg delete \
  'dependencies.@extend-ai/docx-core' \
  'dependencies.@extend-ai/xlsx-core' \
  'dependencies.@extend-ai/vue-docx' \
  'dependencies.@extend-ai/vue-xlsx' \
  'dependencies.@extend-ai/vue-extend'
...
import { XlsxWorkerClient } from "@extend-ai/xlsx-core/runtime"
...
不得从 `dist` 路径导入样式。`@extend-ai/vue-extend/dist/index.css` 仅为已有应用的兼容映射，新代码一律使用 `./style.css`。本地 demo 若保留源码别名，别名只能匹配包根入口，不能把 CSS 子路径错误改写到 `src/index.ts` 下。
