...
同一份 12 页组合素材在 `mode="browse"` 中形成 12 个纵向连续页面，滚动容器具有真实滚动范围；切换 `mode="present"` 后只保留一个单页舞台，播放对象标记正常挂载。`PptxStage` 的页面点击、对象点击和对象右键分别验证了 `selection-change`、`object-click` 和 `context-menu`，参数包含正确的 `slideIndex`、`objectKey` 与坐标。组件测试与 Chromium 真实交互均通过，三浏览器脚本已固定同一组断言。
