...
test("P4 release readiness baseline", () => {
  ...
  try {
    ...
    add(
      ...
      exportChecks,
      "在最终合并后刷新九包导出快照，并确保合同、package.json 与正式 dist 完全一致。",
    );
    ...
    add(
      ...
      Object.values(versions).every((version) => version === "0.6.0"),
      { planned: "0.6.0", actual: versions },
      "在发布流程冻结候选制品前一次性把九包版本升级为已选定的 0.6.0；不得提前零散改版本。",
    );
    ...
    add(
      ...
      "隔离复制当前源码两次，冻结锁文件安装并正式构建九包；逐文件比较有效内容并保存差异。",
    );
    ...
    add(
      ...
      "确认授权来源后，为九包补 license 字段和实际 LICENSE 文件，并验证进入 tgz。",
    );
    ...
    add(
      ...
      "全部 P3 接口冻结后重建九包并刷新经人工确认的公开声明指纹；不能用自动接受变化代替复核。",
    );
