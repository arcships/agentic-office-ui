...
import { resolveDocumentForLayout } from "../layout";
import type {
...
export function buildDocxLayoutDiagnostics(
  ...
  const documentLayout = resolveDocumentForLayout(model);
  const resolvedOptions: LayoutOptions = {
    ...options,
    pageWidth: options.pageWidth ?? documentLayout.layout.pageSizePx.width,
    pageHeight: options.pageHeight ?? documentLayout.layout.pageSizePx.height
  };
  const layoutPages = layoutDocument(model, resolvedOptions);
  const pageWidth = resolvedOptions.pageWidth ?? documentLayout.layout.pageSizePx.width;
  const pageHeight = resolvedOptions.pageHeight ?? documentLayout.layout.pageSizePx.height;
