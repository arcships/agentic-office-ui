...
export function setWasmSource(source: WasmSource): void {
  if (initPromise) {
    throw new Error(
      "docx-core: setWasmSource must be called before the first parse/serialize call initializes WASM"
    );
  }
...
async function defaultWasmSource(): Promise<WasmSource> {
  ...
  if (!response.ok) {
    throw new Error(
      `docx-core: failed to load the bundled WebAssembly binary from ${wasmUrl.href} ` +
    ...
    initPromise = (chosen !== undefined ? Promise.resolve(chosen) : defaultWasmSource())
      ...
      .catch((error: unknown) => {
        if (error instanceof WebAssembly.CompileError) {
          throw new Error(
            "docx-core: the bundled WebAssembly binary failed to compile. It requires " +
