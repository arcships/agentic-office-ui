// ---------------------------------------------------------------------------
// XLSX Core Types — framework-agnostic, zero React/Vue dependency
// Types aligned with the public @extend-ai/react-xlsx API surface
// ---------------------------------------------------------------------------
...
// ── Cell Style Context (no React.CSSProperties) ────────────────────────────

/**
 * Framework-agnostic cell style context.
 * The `resolvedStyle` is typed as a generic record so UI adapters can cast it
 * to their framework's style type (e.g. React.CSSProperties or Vue :style).
 */
export interface XlsxCellStyleContext<TStyle = Record<string, unknown>> {
...
export interface XlsxShapeTextBox {
  ...
  insetPx?: { bottom: number; left: number; right: number; top: number }
...
export interface XlsxShape {
  ...
  sheetIndex: number
  svgPath?: string
  svgViewBox?: { height: number; width: number }
...
export type XlsxFormControlKind =
  | "button" | "checkbox" | "dropdown" | "editbox" | "group-box"
  | "label" | "listbox" | "radio" | "scrollbar" | "spinner" | "unknown"
...
export interface XlsxChart {
  ...
  varyColors?: boolean
  view3d?: { depthPercent?: number; perspective?: number; rAngAx?: boolean; rotX?: number; rotY?: number }
  wireframe?: boolean
  workbookSheetIndex: number
  zIndex: number
}
...
export type XlsxSheetThumbnailResolution = number | { maxHeight?: number; maxWidth?: number }
...
export interface XlsxViewerControllerOptions {
...
export interface XlsxViewerController {
  ...
  getCellSnapshotAsync?: (workbookSheetIndex: number, row: number, col: number) => Promise<{ displayValue: string; formula: string }>
...
export type XlsxWasmSource = string | URL | Request | Response | BufferSource | WebAssembly.Module

export class XlsxFileSizeLimitExceededError extends Error {
  fileSizeBytes: number
  maxFileSizeBytes: number
  constructor(fileSizeBytes: number, maxFileSizeBytes: number) {
    super(`File size ${fileSizeBytes} exceeds limit of ${maxFileSizeBytes} bytes`)
