...
export const MemoSurfaceChartComposite = /* @__PURE__ */ defineComponent({
  name: "MemoSurfaceChartComposite",
  props: {
    background: { type: String, required: true },
    borderColor: { type: String, required: true },
    chart: { type: Object as () => XlsxChart, required: true },
    fontFamily: { type: String, required: true },
    layout: { type: Object as () => SurfaceLayout, required: true },
    onDoubleClick: { type: Function as unknown as () => ((event: MouseEvent) => void) | undefined, default: undefined },
    onPointerDown: { type: Function as unknown as () => ((event: PointerEvent) => void) | undefined, default: undefined },
    overlay: { type: [Object, Array] as unknown as () => VNode | VNode[] | null, default: null },
    palette: { type: Object as () => SurfacePalette, required: true },
    fallback: { type: [Object, Array] as unknown as () => VNode | VNode[] | null, default: null },
  },
  setup(props) {
    const canvasRef = ref<HTMLCanvasElement | null>(null);
    const ready = ref(false);
    const failed = ref(false);
    const isContour = isContourSurfaceChart(props.chart);
    ...
    function drawSurface() {
      ...
      const performDraw = () => {
        try {
          const scene = buildSurfaceMesh(props.chart, props.palette, props.layout);
          if (!scene) {
            if (!cancelled) {
              failed.value = true;
              ready.value = false;
          ...
          canvas.width = Math.max(1, Math.round(props.layout.width * dpr));
          canvas.height = Math.max(1, Math.round(props.layout.height * dpr));
    ...
    watchEffect(() => {
      ...
      const cleanup = drawSurface();
      return cleanup;
    });

    const shouldRenderFallback = isContour || failed.value || !ready.value;

    return () => {
      const { background, borderColor, chart, fontFamily, layout, onDoubleClick, onPointerDown, overlay, fallback: fallbackSlot } = props;
      const shouldFallback = isContourSurfaceChart(chart) || failed.value || !ready.value;
      ...
      return (
        ...
        >
          <canvas
            ref={canvasRef}
            style={{
              display: shouldFallback ? "none" : "block",
              height: "100%",
              inset: 0,
              position: "absolute",
              width: "100%"
          ...
          >
            {shouldFallback ? fallbackSlot : null}
            {overlay}
          </svg>
        </div>
      );
