# @arcships/pptx-core 开发说明

## 当前状态

本包已经提供 PPTX 静态预览、播放模型、文档会话、播放控制器、页面切换和受控媒体，但暂不属于公开发布包，因此 `package.json` 保持 `private: true`。实现范围和验收结果见 [实现设计](../../docs/pptx-playback-implementation-design.md) 与 [正式播放验收记录](../../docs/pptx-playback-acceptance-results.md)。
